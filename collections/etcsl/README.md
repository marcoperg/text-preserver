...
`inventory.py` supports both the historical link-based catalogue and the deposited nested catalogue, where bold identifiers distinguish included compositions from unedited entries. Identifiers remain strings so leading zeros and alphabetic suffixes are preserved.
