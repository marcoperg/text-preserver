...
For `recipe_api = 1`, an inventory adapter exports `analyze_capture(...)`; an optional reader adapter exports `write_static_reader(...)` or `render_static_reader(...)`. Reader generation falls back to `inventory_adapter` when `reader_adapter` is omitted, preserving small and legacy recipes. Corpus parsing, package mappings, and other collection-specific assumptions remain in recipe-local modules rather than the generic engine.
